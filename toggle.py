# Script to TOGGLE (on/off) the power state of a Kasa Smart Plug that uses KLAP LV2 encryption.
...
async def main():
    ...
    try: # Interact with the connected plug.
        ...
        if state_before == 'Off': # Turn plug power on.
          await plug.turn_on()
          await plug.update()
          if plug.is_on == True:
            state_after = 'On'
            ...
            state_after = 'Off'
              ...
              state_after = 'Error'
          msgout = f"Kasa TOGGLE called. Power changed from '{state_before}' to '{state_after}'. Updated custom variable '{COMP_VAR}' in Bitfocus Companion.'"

        else: # Turn plug power off.
          await plug.turn_off()
          await plug.update()
          if plug.is_on == True:
            state_after = 'On'
